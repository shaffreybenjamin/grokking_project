...
#| export
def cross_entropy_high_precision(logits, labels, other_loss=False):
    ...
    if other_loss:
        ...
        logprobs = F.log_softmax(logits[ : , -1].to(torch.float64), dim=-1)
...
def test_logits(logits, p, is_train, is_test, labels, bias_correction=False, original_logits=None, mode='all'):
    ...
    if mode=='train':
        return cross_entropy_high_precision(logits[is_train], labels[is_train], True)
    elif mode=='test':
        return cross_entropy_high_precision(logits[is_test], labels[is_test], True)
    elif mode=='all':
        return cross_entropy_high_precision(logits, labels, True)
